...
class AFPO:
    ...
    def evaluate_generation_zero(self):
        ...
        f = open(f"gate_{self.gate}_stiffness_{self.change_stiffness}_size_{self.change_size}_encoding_{self.encoding}.csv",'w')
        f.write("material_id,fitness\n")
        for i,material in enumerate(self.pop):            
            # Record fitness statistics    
            f.write(f'{material.id},{material.fitness}\n')
